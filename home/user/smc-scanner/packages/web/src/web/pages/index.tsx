...
function Scanner() {
  ...
  const [overlays, setOverlays] = useState<OverlayToggles>(() => lsGet('smc_overlays', {
    ...
    eqLevels: true, oldLevels: true, sessHL: false, trendlines: true,
  }));
  ...
  const [setupStatus, setSetupStatus] = useState<EnhancedSetupStatus | null>(null);
  ...
  // ── Enhanced overlays ──────────────────────────────────────────────────
  const ltfEqLevels = useMemo(
    () => detectEqualLevels(ltfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ltfLen]
  );
  const htfEqLevels = useMemo(
    () => detectEqualLevels(htfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [htfLen]
  );
  const ltfOldLevels = useMemo(
    () => detectOldLevels(ltfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ltfLen]
  );
  const htfOldLevels = useMemo(
    () => detectOldLevels(htfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [htfLen]
  );
  const ltfSessionLevels = useMemo(
    () => detectSessionLevels(ltfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ltfLen]
  );
  const htfSessionLevels = useMemo(
    () => detectSessionLevels(htfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [htfLen]
  );
  const ltfTrendlines = useMemo(
    () => detectTrendlineLiquidity(ltfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ltfLen]
  );
  const htfTrendlines = useMemo(
    () => detectTrendlineLiquidity(htfCandles),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [htfLen]
  );
  const ltfRSI = useMemo(
    () => calculateRSI(ltfCandles, settings.rsiPeriod ?? 14),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ltfLen, settings.rsiPeriod]
  );
  ...
  useEffect(() => {
    ...
    setupTimerRef.current = setTimeout(() => {
      setSetupStatus(validateSetupEnhanced(htfCandles, ltfCandles, settings));
    }, 200);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <ToggleBtn T={T} label="EQL"   active={overlays.eqLevels}  color="#ffcc00" onClick={() => toggleOverlay('eqLevels')} />
        <ToggleBtn T={T} label="OLD"   active={overlays.oldLevels} color="#ff8800" onClick={() => toggleOverlay('oldLevels')} />
        <ToggleBtn T={T} label="S·HL"  active={overlays.sessHL}    color="#8888ff" onClick={() => toggleOverlay('sessHL')} />
        <ToggleBtn T={T} label="TL"    active={overlays.trendlines} color="#00e676" onClick={() => toggleOverlay('trendlines')} />
              ...
              <ChartSizer>
                {(h) => (
                  <CandleChart
                    ...
                    crtLevel={crtLevel}
                    equalLevels={htfEqLevels}
                    oldLevels={htfOldLevels}
                    sessionLevels={htfSessionLevels}
                    trendlines={htfTrendlines}
                    overlays={overlays}
                    theme={T}
                    height={h}
              ...
              <ChartSizer>
                {(h) => (
                  <CandleChart
                    ...
                    crtLevel={crtLevel}
                    equalLevels={ltfEqLevels}
                    oldLevels={ltfOldLevels}
                    sessionLevels={ltfSessionLevels}
                    trendlines={ltfTrendlines}
                    overlays={overlays}
                    theme={T}
                    height={h}
          ...
          <>
            ...
            {setupStatus.tradeZones && (
              <>
                ...
                <StatusItem T={T} label="TP1"
                  value={setupStatus.nearestLiquidityTP
                    ? `${fmt(setupStatus.nearestLiquidityTP)} LIQ`
                    : `${fmt(setupStatus.tradeZones.tp1)} (${setupStatus.tradeZones.rr1}R)`}
                  color={T.bull} />
              </>
            )}
            {setupStatus.rsi && (
              <>
                <div style={{ width: 1, height: 14, background: T.border }} />
                <StatusItem T={T} label="RSI"
                  value={`${setupStatus.rsi.value.toFixed(1)} ${setupStatus.rsi.momentum === 'bullish' ? '▲' : setupStatus.rsi.momentum === 'bearish' ? '▼' : '—'}${setupStatus.rsi.value > 70 ? ' OB' : setupStatus.rsi.value < 30 ? ' OS' : ''}`}
                  color={setupStatus.rsi.value > 70 ? T.bear : setupStatus.rsi.value < 30 ? T.bull : T.label} />
              </>
            )}
          </>
        ) : (
