...
export default function SettingsPanel({ settings, onChange, onClose }: Props) {
  ...
  return (
    <div style={{
      ...
      <Row
        label="RSI PERIOD"
        desc="Lookback period for RSI momentum confluence. 14 is standard. Shown in Scanner tab — not a setup condition."
      >
        <input
          type="number"
          min={2}
          ...
          onChange={e => set('rsiPeriod', Math.max(2, parseInt(e.target.value) || 14))}
          style={inputStyle}
        />
      </Row>
      ...
      <div style={{ padding: '10px 14px' }}>
        <button
          onClick={() => onChange({ fvgMinSize: 0, sweepBuffer: 0, setupExpiryCandles: 30, rsiPeriod: 14 })}
          style={{
            background: 'transparent',
            border: `1px solid ${COLORS.border}`,
            color: COLORS.label,
