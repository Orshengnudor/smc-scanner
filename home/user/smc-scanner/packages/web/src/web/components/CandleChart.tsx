...
import type { FVG, LiquiditySweep, MSS, CRTLevel, EqualLevel, OldLevel, SessionHL, TrendlineLiquidity } from '../lib/smcEngine';
...
const CandleChart = forwardRef<ChartHandle, Props>(({
  ...
  useEffect(() => {
    ...
    if (overlays.eqLevels && equalLevels.length > 0) {
      equalLevels.slice(-6).forEach(eq => {
        const color = eq.type === 'eqh' ? '#ffcc00' : '#66ccff';
        const lineStart = Math.max(eq.time, firstTime);
        const pts = safeLineData(lineStart, lastTime, eq.price);
        if (!pts) return;
        const s = chart.addSeries(LineSeries, {
          color, lineWidth: 1, lineStyle: LineStyle.Dashed,
          priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: false,
          title: eq.type === 'eqh' ? 'EQH' : 'EQL',
        });
        s.setData(pts);
        overlaySeriesRef.current.push(s);
      });
    ...
    // ── Old Highs / Old Lows (unraided liquidity) ─────────────────────────
    if (overlays.oldLevels && oldLevels.length > 0) {
      oldLevels.slice(-8).forEach(ol => {
        const color = ol.raided
          ? (ol.type === 'old_high' ? '#ff880033' : '#44aaff33')
          : (ol.type === 'old_high' ? '#ff8800bb' : '#44aaffbb');
        const lineStart = Math.max(ol.time, firstTime);
        const pts = safeLineData(lineStart, lastTime, ol.price);
        if (!pts) return;
        const s = chart.addSeries(LineSeries, {
          color, lineWidth: ol.raided ? 1 : 2, lineStyle: ol.raided ? LineStyle.Dotted : LineStyle.Dashed,
          priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: false,
          title: ol.type === 'old_high' ? (ol.raided ? 'OldH✗' : 'OldH') : (ol.raided ? 'OldL✗' : 'OldL'),
        });
        s.setData(pts);
        overlaySeriesRef.current.push(s);
      });
    ...
    // ── Session H/L ───────────────────────────────────────────────────────
    if (overlays.sessHL && sessionLevels.length > 0) {
      sessionLevels.slice(-6).forEach(sl => {
        const colorMap: Record<string, string> = {
          'Asia': '#8888ff', 'London': '#00cc77', 'New York': '#ff5555',
        };
        const color = colorMap[sl.session] ?? '#aaaaaa';
        const lineStart = Math.max(sl.startTime, firstTime);
        const lineEnd   = Math.min(sl.endTime, lastTime);
        if (lineStart >= lineEnd) return;
        const hPts = safeLineData(lineStart, lineEnd, sl.high);
        const lPts = safeLineData(lineStart, lineEnd, sl.low);
        [hPts, lPts].filter(Boolean).forEach(pts => {
          const s = chart.addSeries(LineSeries, {
            color: color + 'cc', lineWidth: 1, lineStyle: LineStyle.Solid,
            priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false,
            title: sl.session,
          });
          s.setData(pts!);
    ...
    // ── Trendline Liquidity ───────────────────────────────────────────────
    if (overlays.trendlines && trendlines.length > 0) {
      trendlines.slice(-4).forEach(tl => {
        const color = tl.direction === 'ascending' ? '#00e676' : '#ff5252';
        // Extend trendline from touch1 to lastTime
        const slope = (tl.price2 - tl.price1) / Math.max(1, tl.time2 - tl.time1);
        const extendedPrice = tl.price1 + slope * (lastTime - tl.time1);
        const t1 = Math.max(tl.time1, firstTime);
        const pts: LineData[] = [
          { time: t1 as any, value: tl.price1 + slope * (t1 - tl.time1) },
          { time: lastTime as any, value: extendedPrice },
        ];
        if (t1 >= lastTime) return;
        const s = chart.addSeries(LineSeries, {
          color: color + 'bb', lineWidth: 2, lineStyle: LineStyle.Solid,
          priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: false,
          title: tl.direction === 'ascending' ? 'TL▲' : 'TL▼',
        });
        s.setData(pts);
        overlaySeriesRef.current.push(s);
      });
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      equalLevels, oldLevels, sessionLevels, trendlines,
      overlays.crtLevels, overlays.fvg, overlays.sweep, overlays.mss,
      overlays.eqLevels, overlays.oldLevels, overlays.sessHL, overlays.trendlines,
